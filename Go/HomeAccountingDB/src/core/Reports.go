...
type groupingFunc func(t time.Time, categoryId int, valutaCode string, accountId int) int
type datesFunc func(t time.Time) (entities.NDate, entities.NDate)

func groupByMonth(t time.Time, _ int, valutaCode string, _ int) int {
  return ((t.Year() % 100) * 100 + int(t.Month())) * 100000 + buildHashCode(valutaCode)
}

func groupByCategory(_ time.Time, categoryId int, valutaCode string, _ int) int {
  return categoryId * 100000 + buildHashCode(valutaCode)
}

func groupByAccount(_ time.Time, _ int, _ string, accountId int) int {
  return accountId
}
...
func truncateToMonth(t time.Time) (entities.NDate, entities.NDate) {
  firstOfMonth := time.Date(t.Year(), t.Month(), 1, 0, 0, 0, 0, time.Local)
  return entities.NDate{Value: pq.NullTime{Time: firstOfMonth, Valid: true}}, entities.NDate{Value: pq.NullTime{Time: firstOfMonth.AddDate(0, 1, -1), Valid: true}}
}

func passNil(_ time.Time) (entities.NDate, entities.NDate) {
  return entities.NDate{Value: pq.NullTime{Time: time.Time{}, Valid: false}}, entities.NDate{Value: pq.NullTime{Time: time.Time{}, Valid: false}}
}

func passDate(t time.Time) (entities.NDate, entities.NDate) {
  return entities.NDate{Value: pq.NullTime{Time: t, Valid: true}}, entities.NDate{Value: pq.NullTime{Time: time.Time{}, Valid: false}}
}
...
func BuildReportFromOperations(ops []entities.PrihodRashodByDay, grouping string) ([]entities.ReportItem, error) {
  ...
  for _, op := range ops {
    if groupBy != nil {
      ...
      result = append(result, entities.ReportItem{Date1: entities.NDate{Value:pq.NullTime{Time:op.Date, Valid:true}}, CategoryId: op.CategoryId,
